-- Procesu vadība: izmaiņu vēsture un atjaunošana (auditācija).
...
begin
  if tg_op = 'UPDATE' and old.state is distinct from new.state then
    ...
    where v.module_id = old.id
      and v.id not in (
        select h.id
        from public."Procesu_vadibas_vesture" h
...
for each row
execute function public.pdd_procesu_vadiba_archive_state();
...
begin
  select * into v_hist
  from public."Procesu_vadibas_vesture"
