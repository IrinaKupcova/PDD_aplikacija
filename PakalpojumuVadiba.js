/**
 * Pakalpojumu vadība — uzdevumi, apakšuzdevumi, apakšapakšuzdevumi, Gantt, Lists reģistri.
 * Eksports: globalThis.PDD_PAKALPOJUMU_VADIBA.createPakalpojumuVadibaModule(html, React)
 *
 * STABILA RELĪZE: 202606270 (MODULE_VERSION 9)
 * — Kopīgs stāvoklis Supabase tabulā Pakalpojumu_vadibas_modulis (id = main)
 * — Vēsture: Pakalpojumu_vadibas_vesture; neizslēgt REMOTE_SYNC_ENABLED
...
(function (root) {
  ...
  const REMOTE_TABLE = "Pakalpojumu_vadibas_modulis";
  const REMOTE_TABLE_LEGACY = "Pakalpojumu_vadiba";
  const REMOTE_HISTORY_TABLE = "Pakalpojumu_vadibas_vesture";
  ...
  function uid() {
    return typeof crypto !== "undefined" && crypto.randomUUID
      ? crypto.randomUUID()
      : `pkv_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
  }
  ...
  function saveState(state) {
    ...
    try {
      ...
      console.warn("[Pakalpojumu vadība] Neizdevās saglabāt lokāli", e);
    }
  }
        ...
        if (typeof root.__PDD_ENSURE_DB_SESSION__ !== "function") {
          root.__PDD_ENSURE_DB_SESSION__ = async () => {
            try {
              ...
              if (error) console.warn("[Pakalpojumu vadība] anon auth", error);
            } catch (e) {
              console.warn("[Pakalpojumu vadība] DB sesija", e);
            }
            return true;
          };
        }
   ...
   * ok:false = lasīšanas kļūda (NEDRĪKST bootot tukšu stāvokli uz DB)
   * missing:true = rindas nav (drīkst bootot tikai ja lokāli ir saturs)
   */
  ...
  function shouldPreferRemoteOverLocal(remote, local) {
    ...
    const rt = stateTimestamp(remote);
    const lt = stateTimestamp(local);
    // Jaunākais laiks uzvar — neturam vecāku “bagātāku” lokālo pret jaunāku remote.
  ...
  async function saveRemoteState(sb, state) {
    ...
    // Tukšu stāvokli nekad neaugšupielādējam — tas varēja izdzēst komandas datus.
  ...
  async function recoverRichestHistoryState(sb, current) {
    try {
      const rows = await fetchRemoteHistory(sb, { limit: 30 });
      if (!Array.isArray(rows) || !rows.length) return null;
      let best = null;
      let bestScore = stateContentScore(current);
      for (const row of rows) {
        const st = cloneMigratedState(row?.state);
        if (!st) continue;
        const sc = stateContentScore(st);
        if (sc > bestScore) {
          best = st;
          bestScore = sc;
        }
      ...
      console.warn("[Pakalpojumu vadība] vēstures recovery", e);
      return null;
    }
  }
  ...
  async function restoreRemoteHistory(sb, historyId) {
    ...
    await ensureDbSession(sb);
    const { error } = await sb.rpc("pdd_pakalpojumu_vadiba_atjaunot", { p_history_id: historyId });
    if (error) {
      console.warn("[Pakalpojumu vadība] atjaunošana", error);
      return { ok: false, error };
    }
  ...
  async function hydrateTeamUsersForPv(options = {}) {
    ...
    hydrateTeamUsersPromise = (async () => {
      try {
        ...
        console.warn("[Pakalpojumu vadība] komandas saraksta ielāde", e);
      }
      return getTeamUsers();
    })().finally(() => {
  ...
  function exportPakalpojumuVadibaExcel(phases, workPlanSections) {
    const csv = `\uFEFF${buildPhaseExportLines(phases, workPlanSections)}`;
    downloadTextFile(`pakalpojumu-vadiba-${todayIso()}.csv`, csv, "text/csv;charset=utf-8");
  }

  function exportPakalpojumuVadibaPdf(phases) {
    ...
    const rows = numbered
      ...
      .join("");
    const html = `<!DOCTYPE html><html lang="lv"><head><meta charset="UTF-8"/><title>Pakalpojumu vadība</title>
<style>
...
<h1>Pakalpojumu vadība — eksports</h1>
<p>Datums: ${todayIso()}</p>
...
<script>window.addEventListener("load",function(){setTimeout(function(){window.print()},300)})</script>
  ...
  function ensureStyles() {
    if (typeof document === "undefined" || document.getElementById("pdd-pkv-styles-v9f")) return;
    const el = document.createElement("style");
    el.id = "pdd-pkv-styles-v9f";
    el.textContent = `
      .pv-root {
        --pv-bg: #e8f8f3;
        --pv-surface: #ffffff;
      ...
      .pv-table-wrap {
        overflow: auto; border: 1px solid #c5ebe3; border-radius: 10px; width: 100%;
        max-height: min(70vh, 720px); overscroll-behavior: contain;
      }
      .pv-table {
      ...
      .pv-content-block-view-table {
        width: max-content; min-width: 100%; border-collapse: separate; border-spacing: 0;
        font-size: 0.84rem; table-layout: fixed;
      }
      .pv-table thead th,
      .pv-content-block-view-table thead th {
        position: sticky; top: 0; z-index: 3;
        background: #e8f8f3; box-shadow: inset 0 -1px 0 #c5ebe3;
      }
      ...
      .pv-content-block-view-table { width: max-content; min-width: 100%; border-collapse: separate; border-spacing: 0; font-size: 0.84rem; table-layout: fixed; }
      ...
      .pv-content-block-view-table th {
        background: #e8f8f3; color: #065f46;
        position: sticky; top: 0; z-index: 3;
        box-shadow: inset 0 -1px 0 #c5ebe3;
      }
      .pv-rt-toolbar {
  ...
  function createPakalpojumuVadibaModule(html, React) {
    ensureStyles();
      ...
      useEffect(() => {
        ...
        async function hydrateFromRemote(tryNum = 0) {
          ...
          try {
            ...
            if (!remoteRes.ok) {
              // Lasīšanas kļūda — NEBOOTOJAM tukšu stāvokli uz DB.
            ...
            if (!remote) {
              // Rinda nav — bootojam tikai ja lokāli ir saturs.
              ...
              if (boot?.ok) {
                ...
                console.warn(
                  "[Pakalpojumu vadība] Neizdevās saglabāt Supabase — pārbaudi, vai tabula Pakalpojumu_vadibas_modulis eksistē.",
                  boot?.error,
                );
                if (!cancelled) {
                  setSyncStatus("error");
              ...
              // Ja remote ir tukšs/gandrīz tukšs — mēģinām atjaunot no vēstures (pirms kļūdainas pārrakstīšanas).
              if (stateContentScore(remote) <= stateContentScore(defaultState())) {
                const fromHist = await recoverRichestHistoryState(sb, remote);
                if (fromHist && stateContentScore(fromHist) > stateContentScore(remote)) {
                  const pushHist = await saveRemoteState(sb, fromHist);
                  if (pushHist?.ok) {
                    remote = { ...fromHist, updatedAt: pushHist.updatedAt };
                  } else {
                    remote = fromHist;
                  }
                }
              ...
              if (stateContentScore(remote) > stateContentScore(defaultState()) && pushedRicher) {
                recoveredMsg = "Jaunākie lokālie dati augšupielādēti komandai Supabase.";
              } else if (
                stateContentScore(remote) > stateContentScore(defaultState()) &&
                stateContentScore(local || {}) <= stateContentScore(defaultState())
              ) {
                recoveredMsg = "Dati atjaunoti no Supabase / vēstures.";
              }
            ...
            console.warn("[Pakalpojumu vadība] sākotnējā sinhronizācija", e);
            if (!cancelled) {
              setSyncStatus("error");
        ...
        const poll = setInterval(() => {
          void (async () => {
            ...
            try {
              ...
              console.warn("[Pakalpojumu vadība] fona sinhronizācija", e);
            }
          })();
        }, REMOTE_POLL_MS);
    ...
    function ExportBar({ phases, workPlanSections }) {
      return html`
        <div class="pv-export-bar">
          <button type="button" class="pv-btn" onClick=${() => exportPakalpojumuVadibaExcel(phases, workPlanSections)}>
            ⬇ Excel (CSV)
          </button>
          <button type="button" class="pv-btn" onClick=${() => exportPakalpojumuVadibaPdf(phases)}>⬇ PDF</button>
        </div>
      `;
    }
      ...
      return html`
        <div>
          <nav class="pv-breadcrumb" aria-label="Navigācija">
            <button type="button" class="pv-link" onClick=${onGoOverview}>Pakalpojumu vadība</button>
            <span>›</span>
            ${ancestors.map(
              (a) => html`
                <span key=${a.id}>
  ...
  root.PDD_PAKALPOJUMU_VADIBA = {
    MODULE_VERSION,
    STABLE_RELEASE,
    createPakalpojumuVadibaModule,
    loadState,
    ...
    exportPakalpojumuVadibaExcel,
    exportPakalpojumuVadibaPdf,
    exportGanttExcel,
    exportGanttPdf,
    exportGanttImage,
