# Kompetences saglabāšana — obligāta migrācija (RPC ar "e-mail" kolonnu + self saglabāšana).
#
#   $env:SUPABASE_DB_PASSWORD = 'postgres-parole'
#   .\scripts\apply-kompetence-migration.ps1

$ErrorActionPreference = 'Stop'
$Root = Split-Path $PSScriptRoot -Parent
$Migration = Join-Path $Root 'supabase\migrations\20260618120000_pdd_self_kompetence_by_email.sql'
...
Supabase → Project Settings → Database → postgres parole.

  `$env:SUPABASE_DB_PASSWORD = 'tava-parole'
  .\scripts\apply-kompetence-migration.ps1

Vai SQL Editor: supabase/migrations/20260618120000_pdd_self_kompetence_by_email.sql
"@
}
...
Write-Host "Kompetences migrācija → $hostName" -ForegroundColor Cyan
...
Write-Host 'Gatavs. Kolēģiem: Ctrl+F5 pārlūkā.' -ForegroundColor Green
